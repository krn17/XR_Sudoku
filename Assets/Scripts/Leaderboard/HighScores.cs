...
{
	const string privateCode = "nwdtS6GQ3UGs4SjFpHj-rQTHAoMnHOXUyZmjuMse97_g";  //Key to Upload New Info
	const string publicCode = "6267b8d08f40bbb378d7b7e9";   //Key to download
	...
	public void DownloadScores(DisplayHighscores highscoreDisplay)
	{
		StartCoroutine(DatabaseDownload(highscoreDisplay));
	}
	IEnumerator DatabaseDownload(DisplayHighscores highscoreDisplay)
	{
		...
		if (string.IsNullOrEmpty(www.error))
		{
			...
			highscoreDisplay.SetScoresToMenu(scoreList);
		}
		else print("Error uploading" + www.error);
	}
